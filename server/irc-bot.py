...
IRC_CHANNEL = os.environ.get("IRC_CHANNEL", "#so")
...
class SoundStreamBot(irc.bot.SingleServerIRCBot):
    def __init__(self):
        logger.info(f"Initializing bot: {IRC_NICKNAME} on {IRC_SERVER}:{IRC_PORT}")
        irc.bot.SingleServerIRCBot.__init__(self, [(IRC_SERVER, IRC_PORT)], IRC_NICKNAME, "Music Streaming Bot")
    ...
    def on_welcome(self, connection, event):
        ...
        connection.privmsg(self.channel, "SoundStream bot is now online! Use !request <song> to add a song to the queue.")
        
        # Start the now playing announcement thread
        self.now_playing_thread = threading.Thread(target=self.announce_now_playing_loop, args=(connection,))
    ...
    def handle_song_request(self, connection, requester, query):
        ...
        if not query:
            connection.privmsg(self.channel, f"{requester}: Please provide a song name or YouTube URL")
            return
            
        try:
            response = requests.post(API_REQUEST_URL, json={
                "query": query,
                "requester": requester
            }, timeout=10)
            ...
            if response.status_code == 201:
                data = response.json()
                connection.privmsg(self.channel, f"{requester}: {data['message']}")
            else:
                error_data = response.json()
                connection.privmsg(self.channel, f"{requester}: Failed to add song - {error_data.get('message', 'Unknown error')}")
                
        except Exception as e:
            logger.error(f"Error handling request: {str(e)}")
            connection.privmsg(self.channel, f"{requester}: Sorry, there was an error processing your request")
    ...
    def check_and_announce_now_playing(self, connection):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"Failed to get now playing info: {response.status_code}")
        except Exception as e:
    ...
    def announce_track_info(self, connection, data):
        ...
        if now_playing != self.currently_playing:
            ...
            if now_playing:
                ...
                connection.privmsg(self.channel, "🔇 Nothing is currently playing")
